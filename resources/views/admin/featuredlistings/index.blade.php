        ...
        <div class="kt-portlet__body">
            ...
            <div class="row p-b-1per" >
                ...
                <div class="cold-md-8 text-right">
                    <a href="{{url('admin/create-featured-listings')}}"><button type="button" class="btn btn-primary"><i class="fa fa-plus"></i> Add Featured Listings</button></a>
                </div>
            </div>
            ...
            <div class="kt-timeline-v2__items  kt-padding-top-25 kt-padding-bottom-30">
                <div class="kt-section__content">
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%;">
                            <thead>
                                <tr>
                                	...
                                	<th width="10%;" class="center">Start Time </th>
                                	<th width="10%;" class="center">End Time</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($featuredlistings as $key=>$featuredlisting)
                                    <tr>
                                       
                                        <th scope="row" class="center">{{++$key}}</th>
                                        <td >{{ucfirst($featuredlisting->placement)}}</td>
                                        <td >{{$featuredlisting->charges}}</td>
                                        <td >{{$featuredlisting->start_date}}</td>
                                         <td >{{$featuredlisting->end_date}}</td>
                                         <td >{{$featuredlisting->start_time}}</td>
                                         <td >{{$featuredlisting->end_time}}</td>
                                         
                                         <td >{{$featuredlisting->total}}</td>
                                         <td >
                                          @if($featuredlisting->status == 0)
                                               De Active
                                            @else
                                                Active
                                            @endif
                        ...
                        <td align="center">
                        <a href="{{url('admin/edit-featured-listings/'.$featuredlisting->id)}}"><button type="button" class="btn btn-type btn-primary btn-sm btn-icon" data-placement="top" data-content="Edit"><i class="fa fa-edit"></i></button>
                        <a href="javascript:;" ><button type="button"  class="showDeleteModal btn btn-type btn-danger  btn-sm btn-icon"  data-id="{{$featuredlisting->id}}" data-placement="top" data-content="Delete"><i class="fa fa-trash"></i></button></a>
                        </td>
                                    </tr>
                                @endforeach
                                @else
...
$("#deleteRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/delete-featured-listings/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
