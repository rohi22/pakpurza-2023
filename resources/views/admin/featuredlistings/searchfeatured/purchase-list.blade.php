...
@section('title')
Simsar | Purchases Search Featured Lists
@endsection
        ...
        <div class="kt-portlet__head">
            <div class="kt-portlet__head-label">
                <h3 class="kt-portlet__head-title">
                   Purchases Search Featured Lists
                </h3>
            </div>
                            ...
                            <tbody>
                                ...
                                @foreach ($purchasessearchfeatured as $key=>$p)
                                    <tr>
                                        ...
                                        <th scope="row" class="center"><input type="checkbox" value="{{$p->id}}" name="purchasessearchfeatured[]" /> {{$key + $purchasessearchfeatured->firstItem()}}</th>
                                        <td>
                                           <a href="javascript:;"><span class="showUserModal" data-toggle="kt-popover" data-id="{{$p->user_id}}" data-placement="top" data-content="view">{{$p->user_id}}</i></span></a> 
                                        </td>
                                        <td >Purchase</td>
                                        <td >{{$p->Plan_title}}</td>
...
$(".showUserModal").click(function(){
    ...
    var id = $(this).attr('data-id');
    var row = '<tr><th class="colorRed">User Name</th><th class="colorRed">User Title</th><th class="colorRed">Phone</th><th class="colorRed">Address</th></tr>';
      
      $.get({
          
            
            url  : "{{ url('get-users-detail') }}"+'/'+id,
            success:function(data){
                $('#tbody').html('');
                var res = JSON.parse(data);
                
                if( res.length > 0 ){
                    var address = (res[0].address==null) ?'- - -':res[0].address;
                    row += '<tr><td class="colorRed">'+res[0].name+'</td><td class="colorRed">'+res[0].email+'</td><th class="colorRed">'+res[0].phone+'</th><th class="colorRed">'+address+'</th></tr>';
                    $('#tbody').append(row);
                    
                }else{
                    $('#tbody').append('<tr><td colspan="4"><p style="margin-left:38%">no details found..!</p></td></tr>');
...
$("#approveRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/approve-search-plans/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$("#rejectRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/reject-search-plans/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$('#btbdeleteall').click(function(){
        var values = $("input[name='purchasessearchfeatured[]']:checked")
...
$('#deleteall').click(function(){
      ...
      var values = $("input[name='purchasessearchfeatured[]']:checked")
              .map(function(){return $(this).val();}).get();
                   $.ajax({
        type : "post",
        url  : "{{ url('admin/delete-purchases-fl-all') }}",
        data : {
        "_token": "{{ csrf_token() }}",
       "type_id":values,
