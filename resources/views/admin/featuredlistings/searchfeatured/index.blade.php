            ...
            <div class="row p-b-1per" >
                <div class="col-md-8 text-right"></div>
                 ...
                 <div class="col-md-2 text-right">
                    ...
                    <a href="{{url('admin/create-search-featured-listings')}}"><button type="button" class="btn btn-primary"><i class="fa fa-plus"></i> Add  Search Featured Plans</button></a>
                </div>
            </div>
            </div>
                        ...
                        <table class="table table-bordered" width="100%;">
                            <thead>
                                <tr>
                                    ...
                                    <th width="10%;">No .Of Posting</th>
                                    <th width="10%;">Cost</th>
                                    <th width="10%;">Keyword Limit</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($searchfeaturedlistings as $key=>$searchfeaturedlisting)
                                    ...
                                    <td >{{ucfirst($searchfeaturedlisting->title)}}</td>
                                     <td >{{$searchfeaturedlisting->no_of_post}}</td>
                                      <td >{{$searchfeaturedlisting->cost}}</td>
                                       <td >{{$searchfeaturedlisting->keyword_limit}}</td>
                                       ...
                                       <td>
                                        ...
                                        <td align="center">
                                            <a href="{{url('admin/edit-search-featured-listings/'.$searchfeaturedlisting->id)}}"><button type="button" class="btn btn-type btn-primary btn-sm btn-icon" data-toggle="kt-popover" data-placement="top" data-content="Edit"><i class="fa fa-edit"></i></button>
                                            <a href="javascript:;" ><button type="button"  class="showDeleteModal btn btn-type btn-danger  btn-sm btn-icon"  data-id="{{$searchfeaturedlisting->id}}" data-placement="top" data-content="Delete"><i class="fa fa-trash"></i></button></a>
                                        </td>
                                    </tr>
                                @endforeach
                                @else
...
$("#deleteRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/delete-search-featured-listings/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$('#deleteall').click(function(){
    ...
    var values = $("input[name='searchfeaturedlistings[]']:checked")
              ...
              $.ajax({
                  type : "post",
        url  : "{{ url('delete-searh-featured-listings-all') }}",
        data : {
        "_token": "{{ csrf_token() }}",
