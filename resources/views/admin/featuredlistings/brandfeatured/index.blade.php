        ...
        <div class="kt-portlet__body">
            <div class="row p-b-1per" >
                ...
                <div class="col-md-3 text-right">
                    <a href="{{url('admin/create-type-featured-listings')}}"><button type="button" class="btn btn-primary"><i class="fa fa-plus"></i> Add Type Featured Plans</button></a>
                </div>
            </div>
                ...
                <div class="kt-section__content">
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%;">
                            <thead>
                                <tr>
                                    <th width="5%;"  class="center "><input type="checkbox" name="typefeaturedlistings" id="typefeaturedlistings">#</th>
                                    <th width="25%;">Plan Of Title</th>
                                    <th width="10%;">Type Limit</th>
                                    <th width="10%;">Type Item Limit</th>
                                    <th width="10%;">Cost</th>
                            ...
                            <tbody>
                                @if($searchfeaturedlistings->count() > 0)
                                @foreach ($searchfeaturedlistings as $key=>$searchfeaturedlisting)
                                    <tr>
                                       
                                        <th scope="row" class="center "><input type="checkbox" value="{{$searchfeaturedlisting->id}}" name="typefeaturedlistings[]" > {{$key + $searchfeaturedlistings->firstItem()}}</th>
                                        <td >{{ucfirst($searchfeaturedlisting->title)}}</td>
                                     <td >{{$searchfeaturedlisting->type_limit}}</td>
                                     <td >{{$searchfeaturedlisting->type_item_limit}}</td>
                                      <td >{{$searchfeaturedlisting->cost}}</td>
                                       
                                       
                                       <td>
                                           <?php 
                                           
                                           if($searchfeaturedlisting->status == 0){
                                               ?>
                                        ...
                                        <td align="center">
                                            <a href="{{url('admin/edit-type-featured-listings/'.$searchfeaturedlisting->id)}}"><button type="button" class="btn btn-type btn-primary btn-sm btn-icon" data-toggle="kt-popover" data-placement="top" data-content="Edit"><i class="fa fa-edit"></i></button>
                                            <a href="javascript:;" ><button type="button"  class="showDeleteModal btn btn-type btn-danger  btn-sm btn-icon" data-toggle="kt-popover" data-id="{{$searchfeaturedlisting->id}}" data-placement="top" data-content="Delete"><i class="fa fa-trash"></i></button></a>
                                        </td>
                                    </tr>
                                @endforeach
                                @else
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="kt-scroll ps ps--active-y h-200px" data-scroll="true" data-height="200" >
                    <p id="ajaxData"></p>
                <div class="ps__rail-x l-0px-b-365" ><div class="ps__thumb-x l-0px-w-0px" tabindex="0" ></div></div><div class="ps__rail-y t-365px-r-0px-h-200px" ><div class="ps__thumb-y t-130px-h-70px" tabindex="0" ></div></div></div>
            </div>
...
$("#deleteRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/delete-type-featured-listings/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
        ...
        success:function(data){
           ...
           if(data==1){
            ...
            if(data==0){
              ...
              toastr.success('Plan In Used You Can\'t Delete This Plan', 'Deleted');
...
$('#btbdeleteall').click(function(){
        var values = $("input[name='typefeaturedlistings[]']:checked")
...
$('#deleteall').click(function(){
    var values = $("input[name='typefeaturedlistings[]']:checked")
              .map(function(){return $(this).val();}).get();
               $.ajax({
                   type : "post",
        url  : "{{ url('delete-type-featured-listings-all') }}",
        data : {
        "_token": "{{ csrf_token() }}",
       "type_id":values,
...
$(document).ready(function(){
        $('#typefeaturedlistings').click(function(){
            if($(this).prop("checked") == true){
                $("[name='typefeaturedlistings[]']").attr("checked", "checked");
            }
            else if($(this).prop("checked") == false){
                $("[name='typefeaturedlistings[]']").removeAttr("checked");
            }
        });
    });
