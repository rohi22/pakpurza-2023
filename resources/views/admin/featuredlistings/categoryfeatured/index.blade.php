...
@section('title')
Simsar | Category Featured  Plans
@endsection
        ...
        <div class="kt-portlet__body">
            <div class="row p-b-1per" >
                ...
                <div class="col-md-3 text-right">
                    <a href="{{url('admin/create-category-featured-listings')}}"><button type="button" class="btn btn-primary"><i class="fa fa-plus"></i> Add Category Featured  Plans</button></a>
                </div>
            </div>
                ...
                <div class="kt-section__content">
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%;">
                            <thead>
                                <tr>
                                    <th width="5%;"  class="center"><input type="checkbox" name="searchfeaturedlistings" id="searchfeaturedlistings">#</th>
                                    <th width="25%;">Plan Of Title</th>
                                    <th width="20%;">Category Limit</th>
                                    <th width="15%;">Category Item Limit</th>
                                    <th width="5%;">Cost</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($searchfeaturedlistings as $key=>$searchfeaturedlisting)
                                    ...
                                    <th scope="row" class="center"><input type="checkbox" value="{{$searchfeaturedlisting->id}}" name="searchfeaturedlistings[]" >   {{$key + $searchfeaturedlistings->firstItem()}}</th>
                                    <td >{{ucfirst($searchfeaturedlisting->title)}}</td>
                                    <td >{{$searchfeaturedlisting->category_limit}}</td>
                                    <td >{{$searchfeaturedlisting->category_item_limit}}</td>
                                    <td >{{$searchfeaturedlisting->cost}}</td>
                                       ...
                                       <td>
                                        ...
                                        <td align="center">
                                            <a href="{{url('admin/edit-category-featured-listings/'.$searchfeaturedlisting->id)}}"><button type="button" class="btn btn-type btn-primary btn-sm btn-icon" data-toggle="kt-popover" data-placement="top" data-content="Edit"><i class="fa fa-edit"></i></button>
...
$("#deleteRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/delete-category-featured-listings/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$('#btbdeleteall').click(function(){
        var values = $("input[name='searchfeaturedlistings[]']:checked")
...
$('#deleteall').click(function(){
        var values = $("input[name='searchfeaturedlistings[]']:checked")
              .map(function(){return $(this).val();}).get();
               $.ajax({
                    type : "post",
        url  : "{{ url('delete-category-featured-listings-all') }}",
        data : {
        "_token": "{{ csrf_token() }}",
       "searchfeaturedlistings_id":values,
...
$(document).ready(function(){
        $('#searchfeaturedlistings').click(function(){
            if($(this).prop("checked") == true){
                $("[name='searchfeaturedlistings[]']").attr("checked", "checked");
            }
            else if($(this).prop("checked") == false){
                $("[name='searchfeaturedlistings[]']").removeAttr("checked");
            }
        });
    });
