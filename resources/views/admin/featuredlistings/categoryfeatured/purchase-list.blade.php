...
@section('title')
Simsar | Purchases Category Featured List
@endsection
        ...
        <div class="kt-portlet__head">
            <div class="kt-portlet__head-label">
                <h3 class="kt-portlet__head-title">
                    Purchases Category Featured List
                </h3>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
           <input type="hidden" id="deleteId">
            <div class="modal-body p-t-5per-p-b-11per" >
                <center>
                    <i class="la la-info-circle c-fb-f-s-70ps"  ></i><br><br>
                    <h3>Are you sure?</h3>
                    <P>You won't be able to revert this!</P>
                    <div >
                        <button type="button" class="btn btn-primary" data-dismiss="modal">No, cancel!</button>
...
$("#approveRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/approve-category-plans/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$("#rejectRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/reject-category-plans/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$('#deleteall').click(function(){
      var values = $("input[name='purchasessearchfeatured[]']:checked")
              .map(function(){return $(this).val();}).get();
                   $.ajax({
        type : "post",
        url  : "{{ url('delete-purchasessearchfeatured-all') }}",
        data : {
        "_token": "{{ csrf_token() }}",
       "type_id":values,
