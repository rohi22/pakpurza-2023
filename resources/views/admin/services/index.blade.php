    ...
    <div class="kt-portlet kt-portlet--height-fluid">
        ...
        <div class="kt-portlet__body">
            <div class="row p-b-1per">
                   ...
                   <div class="col-md-2 text-right">
                    <a href="{{url('admin/create-services')}}"><button type="button" class="btn btn-primary"><i class="fa fa-plus"></i> Add Service</button></a>
                </div>
                ...
                <div class="kt-section__content">
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%;">
                            <thead>
                                <tr>
                                    <th width="5%;"  class="center"><input type="checkbox" name="services" id="services">#</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($services as $key=>$s)
                                    <tr>
                                        ...
                                        <th scope="row" class="center"><input type="checkbox" value="{{$s->id}}" name="services[]" >   {{$key + $services->firstItem()}}</th>
                                        <td class="center">{{$s->title}}</td>
                                        <td class="center">{{$s->price}}</td>
                                         <td class="center">
                                          @if($s->status == 0)
                                               De Active
                                            @else
                                         ...
                                         <td align="center">
                                            <a href="{{url('admin/edit-services/'.$s->id)}}"><button type="button" class="btn btn-brand btn-primary btn-sm btn-icon"  data-placement="top" data-content="Edit"><i class="fa fa-edit"></i></button></a>
                                            <a href="javascript:;" ><button type="button"  class="showDeleteModal btn btn-brand btn-danger  btn-sm btn-icon" data-toggle="kt-popover" data-id="{{$s->id}}" data-placement="top" data-content="delete"><i class="fa fa-trash"></i></button></a>
                                        </td>
                                    </tr>
                                @endforeach
                                @else
...
$("#deleteRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/delete-services-ads/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
        ...
        success:function(data){
           ...
           if(data==1){
            ...
            toastr.success('Service in used You Can not delete', 'Deleted');
...
$('#btbdeleteall').click(function(){
        var values = $("input[name='services[]']:checked")
...
$(document).ready(function(){
        $('#services').click(function(){
            if($(this).prop("checked") == true){
                $("[name='services[]']").attr("checked", "checked");
            }
            else if($(this).prop("checked") == false){
                $("[name='services[]']").removeAttr("checked");
            }
        });
    });
