...
@section('title')
Simsar | Sub Categories Attributes
@endsection
        ...
        <div class="kt-portlet__head">
            <div class="kt-portlet__head-label">
                <h3 class="kt-portlet__head-title">
                    Attributes for <a href="{{ url('sub-categories/'.$subCategory->category_id) }}">{{ucfirst($subCategory->title)}}</a>
                </h3>
            </div>
        ...
        <div class="kt-portlet__body">
             <div class="row" style="padding-bottom:1%;">
                  <div class="col-md-12 text-right">
                      <button type="button" class="btn btn-danger" id="deletebtnall"><i class="fa fa-trash"></i>Delete</button>
                      </div>
                 </div>
            
            <!--<div class="row" style="padding-bottom:1%;">-->
            <!--    <div class="col-md-12 text-right">-->
            <!--        <a href="{{url('create-attribute/'.$subCategory->id)}}"><button type="button" class="btn btn-primary"><i class="fa fa-plus"></i> Add Attributes For {{ucfirst($subCategory->title)}}</button></a>-->
            <!--    </div>-->
            <!--</div>-->
                                    ...
                                    @foreach ($attributes as $key=>$attribute)
                                        <tr>
                                            ...
                                            <th scope="row" class='center'>
                                                 <input type="checkbox" value="{{$attribute->id}}" name="attrid[]" >
                                                {{++$key}}</th>
                                            <td >{{ucfirst($attribute->att_title)}}</td>
                                            <td align="center">
                                                @if(!empty($attribute->att_id))
                                                    @if($attribute->att_id==1)
                                                        <select name="" id="" class="form-control">
                                                         <!--<option>Drop Down</option>-->
                                                            <option value="">--Drop Down--</option>
                                                            </select></span>
                                                    @elseif($attribute->att_id==2)
                                                           <!--<span class="btn btn-sm btn-primary">ucfirst(Button)</span>-->
                                                        <span class="btn btn-sm btn-primary">Button</span>
                                                    @elseif($attribute->att_id==3)
                                                    <!--<input type="text" disabled value="Text Bos">-->
                                                        <input type="text" disabled value="Test Box" class="form-control">
                                                    @endif
                                                @endif
                                            </td>
                                            <td align="center"><a href="javascript:;" class="descriptionModal" data-id="{{$attribute->id}}"><i class="fa fa-eye"></i></a></td>
                                            <td align="center">
                                                <!--<a href="{{url('edit-attribute/'.$attribute->id)}}"><button type="button" class="btn btn-type btn-primary btn-sm btn-icon"  data-placement="top" data-content="Edit"><i class="fa fa-edit"></i></button>-->
                                                <a href="javascript:;" ><button type="button"  class="showDeleteModal btn btn-type btn-danger  btn-sm btn-icon" data-id="{{$attribute->id}}" data-placement="top" data-content="Delete"><i class="fa fa-trash"></i></button></a>
                                                <!--<a href="{{ url('create-property/'.$attribute->id) }}" ><button type="button"  class="btn btn-type btn-info  btn-sm btn-icon"  data-placement="top" data-content="Add Properties"><i class="fa fa-parking "></i></button></a>-->
                                            </td>
                                        </tr>    
                                    @endforeach
                                @else
...
$('#deletebtnall').click(function(){
     ...
     var values = $("input[name='attrid[]']:checked")
