        ...
        <div class="kt-portlet__head">
            <div class="kt-portlet__head-label">
                <h3 class="kt-portlet__head-title">
                    Types for <a href="{{ url('sub-categories/'.$subCategory->category_id) }}">{{ucfirst($subCategory->title)}}</a>
                </h3>
            </div>
        ...
        <div class="kt-portlet__body">
               <div class="row p-b-1per" >
                <div class="col-md-12 text-right">
                    <button type="button"  class="btn btn-type btn-danger deletemodal" ><i class="fa fa-trash"></i>Delete</button>
                </div>
            </div>
                ...
                <div class="kt-section__content">
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%;">
                            ...
                            <tbody>
                                @if($ad_brands->count() > 0)
                                    @foreach ($ad_brands as $key=>$b)
                                        <tr>
                                            <!--<input type="hidden" id="description{{$b->id}}" value="{{$b->description}}">-->
                                             
                                            <th scope="row" class="center">
                                                <input type="checkbox" value="{{$b->id}}" name="subcatbrandid[]" >{{++$key}}</th>
                                            <td >{{ucfirst($b->att_title)}}</td>
                                            <td align="center">
                                                <!--<a href="{{url('edit-attribute/'.$b->id)}}"><button type="button" class="btn btn-type btn-primary btn-sm btn-icon"  data-placement="top" data-content="Edit"><i class="fa fa-edit"></i></button>-->
                                                ...
                                                <!--<a href="{{ url('create-property/'.$b->id) }}" ><button type="button"  class="btn btn-type btn-info  btn-sm btn-icon"  data-placement="top" data-content="Add Properties"><i class="fa fa-parking "></i></button></a>-->
                                            </td>
                                        </tr>    
                                    @endforeach
                                @else
...
$("#deleteRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/sub-delete-brands') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$('.deletemodal').click(function(){
    var values = $("input[name='subcatbrandid[]']:checked")
...
$('#deleteall').click(function(){
     var values = $("input[name='subcatbrandid[]']:checked")
              .map(function(){return $(this).val();}).get(); 
               $.ajax({
        type : "post",
        url  : "{{ url('admin/sub-deleteall-brands') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "subcat_brandid": values,
