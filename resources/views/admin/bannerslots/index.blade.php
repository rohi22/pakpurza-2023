        ...
        <div class="kt-portlet__body">
            ...
            <div class="row p-b-1per" >
                ...
                <div class="col-md-10 text-right">
                   <button type="button" class="btn btn-danger" id="btbdeleteall">
                       <i class="fa fa-trash"></i>delete</button></a>
                </div>
                   <div class="col-md-2 text-right"> 
                    <a href="{{url('admin/create-banner-slots')}}">
                        <button type="button" class="btn btn-primary">
                            <i class="fa fa-plus">
                            
                        </i> Add Banner Slots</button></a>
                </div>
            </div>
                ...
                <div class="kt-section__content">
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%;">
                            <thead>
                                <tr>
                                    <th width="5%;"  class="center"><input type="checkbox" name="bannerslots" id="bannerslots">#</th>
                                    <th width="10%;" class="center">Page Name</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($bannerslots as $key=>$b)
                                    <tr>
                                        ...
                                        <th scope="row" class="center"> <input type="checkbox" value="{{$b->id}}" name="bannerslots[]" > {{$key + $bannerslots->firstItem()}}</th>
                                        <td >{{ucfirst($b->page_title)}}</td>
                                        <td >{{ucfirst($b->title)}}</td>
                                        <!--<td >{{$b->price}}</td>-->
                                       
                        <td align="center">
                        <a href="{{url('admin/edit-banner-slots/'.$b->id)}}">
                            <button type="button" class="btn btn-type btn-primary btn-sm btn-icon" data-placement="top" data-content="Edit" title="edit">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="kt-scroll ps ps--active-y h-200px-o-h" data-scroll="true" data-height="200" >
                    <p id="ajaxData"></p>
                <div class="ps__rail-x l-0px-b-365px" >
                    <div class="ps__thumb-x l-0px-w-0px" tabindex="0" ></div>
                </div>
                <div class="ps__rail-y t-365px-r-0px-h-200px" >
        ...
        <div class="modal-content">
           <input type="hidden" id="deleteId">
            <div class="modal-body p-t-5per-p-b-11per"  >
                <center>
                    <i class="la la-info-circle c-fb-f-z-70px"></i><br><br>
                    <h3>Are you sure?</h3>
...
$("#deleteRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/delete-banner-slots/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$('#btbdeleteall').click(function(){
        var values = $("input[name='bannerslots[]']:checked")
...
$('#deleteall').click(function(){
      var values = $("input[name='bannerslots[]']:checked")
              .map(function(){return $(this).val();}).get();
                   $.ajax({
        type : "post",
        url  : "{{ url('delete-banner-slots-all') }}",
        data : {
        "_token": "{{ csrf_token() }}",
       "banner_id":values,
...
$(document).ready(function(){
        $('#bannerslots').click(function(){
            if($(this).prop("checked") == true){
                $("[name='bannerslots[]']").attr("checked", "checked");
            }
            else if($(this).prop("checked") == false){
                $("[name='bannerslots[]']").removeAttr("checked");
            }
        });
    });
