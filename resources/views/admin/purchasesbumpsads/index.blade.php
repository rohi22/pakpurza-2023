                ...
                <div class="kt-section__content">
                    <div class="table-responsive">
                        <table class="table table-bordered" width="100%;">
                            ...
                            <tbody>
                                @if($purchasesbumads->count() > 0)
                                @foreach ($purchasesbumads as $key=>$p)
                                    <tr>
                                       
                                        <th scope="row" class="center"> <input type="checkbox" value="{{$p->id}}" name="purchasesbumads[]" >{{$key + $purchasesbumads->firstItem()}}</th>
                                        <td>
                                        ...
                                        else if($p->payment_type == 4){
                                            echo "Coin Wallet";
                                        }
                                        else if($p->payment_type == 5){
                                            echo "Credit Card";
                                        }
                                        else if($p->payment_type == 6){
                                            echo "Dedit Card";
                                        }
                                        else{
                                            echo "Other";
...
$("#approveRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/approve-bumpup-plans/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$("#rejectRecord").click(function(){
    ...
    $.ajax({
        type : "post",
        url  : "{{ url('admin/reject-bumpup-plans/') }}",
        data : {
        "_token": "{{ csrf_token() }}",
        "id": id,
...
$('#btbdeleteall').click(function(){
        var values = $("input[name='purchasesbumads[]']:checked")
